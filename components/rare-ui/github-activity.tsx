// Local fork of Rare UI GitHub Activity. MIT, (c) 2026 Swami Malode.
// Upstream 539567414bac024260ed696c9647c737380514a7. See FORK.md.
"use client";
...
function describeDay({ count, date, label }: Contribution) {
...
async function fetchCalendar(login: string) {
  ...
  const days: ApiDay[] = ((await res.json()) as {contributions?: ApiDay[]})?.contributions ?? [];
...
const ContributionGrid = ({
  ...
  return (
    <div
      ...
      role="group"
      aria-label={label}
      className="relative rare-calendar-grid"
    >
      {showMonths && (
        <motion.div
          ...
          <div key={weekIndex} className="flex flex-col" style={{ gap }}>
            {week.map((day) => (
              <motion.button
                ...
                onFocus={(event) => { const cell=event.currentTarget.getBoundingClientRect(); setHovered({day,x:cell.left+cell.width/2,y:cell.top}); }}
                onBlur={() => setHovered(undefined)}
                onKeyDown={(event) => {
                  const delta=event.key === "ArrowRight"?7:event.key === "ArrowLeft"?-7:event.key === "ArrowDown"?1:event.key === "ArrowUp"?-1:0;
                  if(delta) { event.preventDefault(); const all=ref.current?.querySelectorAll<HTMLButtonElement>("button"); const index=Array.from(all??[]).indexOf(event.currentTarget); const target=all?.[index+delta]; if(target&&!target.disabled)target.focus(); }
                }}
                key={day.date}
...
const GitHubActivity = ({
  ...
  return (
    <div
      ...
      {...props}
    >
      {!hideHeading && <p className="mb-4 text-base font-medium text-foreground px-1.5">
        {heading}
      </p>}
