...
import { ArrowUpRight, Check, ChevronDown, Clock3, CornerDownRight, Moon, Sun, Terminal, X, RefreshCw, CircleHelp } from 'lucide-react';
...
function PostRow({ post, now }: { post: Post; now: number }) {
    ...
    return <article className="post-row" id={`post-${post.id}`}>
        ...
        <div className="post-main">
            ...
            <div className="post-bottom"><span>@{post.author}<span className="separator">/</span>{post.parentId ? 'reply' : 'post'}</span><button className="text-button" onClick={() => setExpanded(!expanded)} aria-expanded={expanded} aria-controls={`detail-${post.id}`}>{expanded ? 'Less' : post.parentId ? 'View context' : 'Read post'}<ChevronDown size={13} className={expanded ? 'turned' : ''}/></button><a href={post.url} target="_blank" rel="noreferrer" className="source-link" aria-label={`Open ${LABELS[post.category].toLowerCase()} on X`}><ArrowUpRight size={15}/></a></div>
            {expanded && <div className="post-detail" id={`detail-${post.id}`}>
...
export default function Monitor({ initial, renderedAt }: { initial: Snapshot; renderedAt: number }) {
    ...
    const latest = data.posts.find(p => p.category === 'confirmed');
    ...
    return <TooltipProvider delayDuration={100}><main className="monitor-shell">
        <header className="site-header"><Link className="wordmark" href="/" aria-label="Reset Monitor home"><span className="logo"><Terminal size={19} strokeWidth={1.7}/></span><span>reset monitor<span className="brand-product">codex</span></span></Link><button className="icon-button" onClick={toggleTheme} aria-label={dark?'Switch to light theme':'Switch to dark theme'}>{dark?<Sun size={18}/>:<Moon size={18}/>}</button></header>
        <div className="intro-line"><p>Every reset. The updates in between.</p><span className="tracking"><span className={stale?'status-dot stale':'status-dot'}/>{stale?'Updates delayed':'Monitoring'}</span></div>
        <section className="status-panel" aria-label="Current reset status">
            <div className="status-heading"><span className="eyebrow"><Clock3 size={14}/>{activeUpcoming ? activeUpcoming.category==='correction'?'Latest correction':'Up next':'Latest reset'}</span><span className="tiny-code">/ status</span></div>
            <h1>{awaiting ? 'Awaiting confirmation.' : activeUpcoming ? activeUpcoming.timing || (activeUpcoming.category==='correction'?'Reset update':'Reset announced') : latest?'Limits reset.':'Checking for resets'}</h1>
            ...
            <div className="status-footer"><span><span className="last-label">Last confirmed</span>{latest?<a href={latest.url} target="_blank" rel="noreferrer">{dateLabel(latest.at,true)}<ArrowUpRight size={12}/></a>:'Not available'}</span><span className="last-relative">{latest?relative(latest.at,now):''}</span></div>
        </section>
        <section className="activity-section" aria-labelledby="activity-title"><div className="section-heading"><h2 id="activity-title">Reset activity</h2><Select value={year} onValueChange={v=>{setYear(v);setSelected(null);}}><SelectTrigger aria-label="Activity period" className="year-select"><SelectValue>{year==='recent'?'Past year':year}</SelectValue></SelectTrigger><SelectContent>{['recent',...years].map(y=><SelectItem key={y} value={y}>{y==='recent'?'Past year':y}</SelectItem>)}</SelectContent></Select></div>
            <div className="calendar-frame"><div className="day-labels" aria-hidden="true"><span>Mon</span><span>Wed</span><span>Fri</span></div><div className="calendar-scroll" ref={scroll} tabIndex={0} aria-label="Reset calendar, scroll to see older dates">
                <GitHubActivity className="reset-activity" hideHeading showMonths months={13} cellSize={11} selectedDate={selected} onDayClick={selectDay} accent="var(--green-fill)" contributions={days.map(day=>{
