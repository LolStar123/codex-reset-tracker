...
export async function collectSnapshot(before: Snapshot, sources = defaults, started = new Date().toISOString()) {
    ...
    try {
        ...
        } catch { issues.push('Historical source unavailable'); }
        ...
        const snapshot = reconcileArchive({...before, posts:[...posts.values()], checkedAt:started,lastAttemptAt:started,historyCheckedAt,
        ...
        return {snapshot,raw:[...newest.raw,...(backlog?.raw ?? [])],status:issues.length ? 'partial' : 'ok'};
    } catch (error) {
