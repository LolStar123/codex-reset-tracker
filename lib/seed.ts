...
export function seedSnapshot():Snapshot {
    ...
    return reconcileArchive({posts:ordered,events:deriveEvents(ordered),checkedAt:timeline.fetchedAt,historyCheckedAt:history.meta.generated_at,coverageStart:ordered.at(-1)?.at??null,replyCoverageStart:raw.filter(p=>p.author.screen_name==='thsottiaux').map(p=>new Date(p.created_at).toISOString()).sort()[0]??null,error:null});
}
