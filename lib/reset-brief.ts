...
export function resetBrief(posts:Post[],now:number,stale=false):{text:string;url?:string} {
    if(stale)return {text:'source check delayed. hanging on to the last update.'};
    ...
    if(!pending)return {text:'no next reset announced.'};
    const correction=ordered.find(p=>p.category==='correction'&&p.at>pending.at&&kinds(p).some(k=>kinds(pending).includes(k)));
    if(correction)return {text:/cancelled|canceled|not happening|no reset|won.t|will not/i.test(correction.text)?'reset called off. no new date yet.':'reset plans changed. waiting for an update.',url:correction.url};
    ...
    if(hours) {
        ...
        return {text:left>0?`${subject} expected within ${Math.ceil(left/3600000)}h.`:'waiting for reset confirmation.',url:pending.url};
    }
    ...
    if(offset!==null) {
        ...
        if(now>=date+DAY)return {text:'waiting for reset confirmation.',url:pending.url};
        const day=weekdays[new Date(date).getUTCDay()];
        return {text:`${subject} expected ${day}. no time yet.`,url:pending.url};
    }
    if(now-at>7*DAY)return {text:'waiting for reset confirmation.',url:pending.url};
    return {text:`${subject} on the way. no time yet.`,url:pending.url};
}
