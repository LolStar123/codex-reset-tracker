...
test('weekday promises give no invented hourly countdown and expire honestly',()=>{
 assert.equal(resetBrief([promise],now).text,'reset expected tuesday. no time yet.');
 assert.equal(resetBrief([promise],Date.parse('2026-09-23T12:00:00Z')).text,'waiting for reset confirmation.');
 assert.equal(resetBrief([],now).text,'no next reset announced.');
});
test('explicit hourly window counts down, never becomes confirmation',()=>{
 ...
 assert.equal(resetBrief([timed],now+4*3600000).text,'waiting for reset confirmation.');
});
...
test('banked delivery does not erase a pending full reset, full delivery does',()=>{
 ...
 assert.equal(resetBrief([{...bank,resetType:'regular'},promise],now).text,'no next reset announced.');
});
test('corrections and collection failure take precedence over optimistic wording',()=>{
 ...
 assert.equal(resetBrief([cancellation,promise],now).text,'reset called off. no new date yet.');
